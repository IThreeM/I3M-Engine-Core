...
- `i3m-template` improvements to generate projects, that supports code hot reloading.
...
- Use `workspace.dependencies` in the projects generated by `i3m-template` to simplify dependency change.
...
- New `i3m-scripts` crate + flying camera controller script.
- Ability to map ui key back to winit + change key binding property editor.
- Fallback to root directory if `i3m-template script` cant find `game/src`.
...
- Ability to specify vcs for a new project in `i3m-template`
- Set `resolver = 2` for workspaces generated by `i3m-template`
...
- Added `try` reserved keyword for `i3m-template`.
...
- Added support for wasm in i3m-template - now i3m-template generates `executor-wasm` crate which is a special
...
- Add more information to panic messages in `i3m-template` (kudos to [@lenscas](https://github.com/lenscas))
- Check for reserved names in `i3m-template` (kudos to [@TheEggShark](https://github.com/TheEggShark))
...
editor.
- Added missing `Reflect` implementation for scripts made in `i3m-template`.
- Added dependencies optimization for projects generated in `i3m-template`.
